...
function execute(obj) {
    ...
    switch (validatorFunction) {
        ...
        case 'futureDateValidatorNotebook':
            return futureDateValidatorNotebook(fieldValue, item);
    }
}
...
function futureDateValidatorNotebook(value, validationItem) {
    ...
    const currentValue = moment(value, 'DD/MM/YYYY');
    const formatedValue = moment(currentValue).format("MM/DD/YYYY");
    const isValid = value ? new Date(formatedValue) < new Date() : true;
    let errorMessage = validationItem.message ? validationItem.message : `Invalid entry`;
    return {
        isValid,
        errorMessage
    }
